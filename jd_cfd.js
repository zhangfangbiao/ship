...
!(async () => {
    ...
    await requestAlgo();
    await $.wait(1000)
    let res2 = await getAuthorShareCode('https://raw.githubusercontent.com/zhangfangbiao/sharecode/main/jd_cfd.json')
    if (!res2) {
        await $.wait(1000)
        res2 = await getAuthorShareCode('https://gitee.com/zhangfangbiao/sharecode/raw/main/jd_cfd.json')
    }
    $.strMyShareIds = [...(res2 || [])]
...
function shareCodesFormat() {
    return new Promise(async resolve => {
        ...
        // }
        // console.log(`第${$.index}个京东账号将要助力的好友${[]}`)
        resolve();
    })
}
